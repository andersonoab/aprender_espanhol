...
window.WalkEngine = (function () {
  ...
  function updateMetadata(en, pt) {
    ...
    try {
      ...
      navigator.mediaSession.metadata = new window.MediaMetadata({
        ...
        album: "Treino de espanhol · Igarapé Digital",
  ...
  function ttsUrl(text, lang) {
    ...
    const tl = String(lang).startsWith("es") ? "es" : "en";
  ...
  function buildQueue(card, pt, nextCard) {
    ...
    if (nextCard && nextCard.en) {
      q.push({ ghost: true, text: nextCard.en, lang: "es-ES", rate: prefs.rate });
    }
    return q;
  }
  ...
  function wireUi() {
    ...
    bind("pocketWakeLock", "click", (e) => { e.stopPropagation(); wakePocket(); togglePocketWakeLock(); });
