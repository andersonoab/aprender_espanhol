...
window.BridgeMode = (function () {
  ...
  async function renderPrompt() {
    ...
    box.innerHTML = `
      <div class="bridge-eyebrow">Traduza para o espanhol</div>
      <div class="bridge-pt" id="bridgePtText">Carregando enunciado…</div>
      <div class="bridge-actions">
        <button class="bridge-btn" id="bridgeHearPt"><i class="fa fa-headphones"></i> Ouvir em inglês</button>
    ...
    if (hear) hear.onclick = () => originals.speak(pt, "en-US", 1.05);
  ...
  function revealAnswer() {
    ...
    if (box) {
      box.style.display = "block";
      box.innerHTML = `<span class="bridge-answer-label">ES</span> ${textEn}`;
    }
    originals.speak(textEn, "es-ES", 0.95);
  }
  ...
  function decorate() {
    ...
    if (label) {
      label.textContent = prefs.active
        ? "Monte a frase em espanhol: toque ou arraste as palavras."
  ...
  function updateButton() {
    ...
    if (btn) {
      btn.innerHTML = prefs.active
        ...
        : '<i class="fa fa-language"></i> Modo Ponte EN→ES';
  ...
  function install() {
    ...
    window.checkComplete = function () {
      ...
      if (prefs.active && after === "block" && before !== "block") {
        suppressEnUntil = 0;
        setTimeout(() => originals.speak(textEn, "es-ES", 0.98), 150);
      }
    };

    // Ao carregar uma frase no Modo Ponte, o app fala o espanhol por
    // padrão — isso entregaria a resposta. Trocamos pelo inglês.
    window.speak = function (txt, lang = "es-ES", rate = 1) {
      if (prefs.active && Date.now() < suppressEnUntil && String(lang).startsWith("es")) {
        const pt = (currentCard && currentCard.pt) || textPt;
        if (pt) return originals.speak(pt, "en-US", 1.05);
        return;
      }
