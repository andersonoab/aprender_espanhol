...
window.DataLoader = (function () {
  ...
  const URL_FRASES =
    "https://raw.githubusercontent.com/andersonoab/aprender_espanhol/refs/heads/main/frases_unicas_espanhol.txt";
  ...
  const URL_LOCAL = "./frases_unicas_espanhol.txt";
  ...
  async function loadOnline() {
    ...
    if (!list.length) {
      say("<strong>Nenhuma frase reconhecida.</strong> O arquivo chegou, mas nenhuma linha virou par espanhol e inglês.", "erro");
      return;
    }
  ...
  async function loadLocal() {
    ...
    try {
      ...
      say("<strong>Não achei a cópia embutida das frases.</strong> " +
          "Verifique se <code>frases_unicas_espanhol.txt</code> está na mesma pasta do app.", "erro");
      return;
    }
    if (!res || !res.ok) {
