...
function speakQueued(txt, lang = 'es-ES', rate = 1) {
...
function setPlayAudioHandlers() {
  document.getElementById("playAudioEn").onclick = () => speak(textEn, "es-ES", 1);
...
function loadSentence(card) {
  ...
  if (autoReadMode) {
    ...
    setTimeout(() => speak(textEn, "es-ES", 1), 200);
  }
...
function initSpeechRecognition() {
  ...
  speechRecognition.onstart = () => {
    const el = document.getElementById('speechStatus');
    if (el) el.textContent = 'Ouvindo... fale a frase em espanhol.';
  };
...
document.getElementById("loadOnlineBtn").onclick = () => {
  fetch("https://raw.githubusercontent.com/andersonoab/aprender_espanhol/refs/heads/main/frases_unicas_espanhol.txt")
    .then(res => res.text())
    .then(txt => {
